...
test('assigned revision for first block matches earliest revision that contains it (should fail)', async ({ page, request }) => {
  ...
  for (let i = 0; i < blockCount; i++) {
    const text = (await blocks.nth(i).innerText()).slice(0, 200).replace(/\s+/g, ' ').trim().toLowerCase();
